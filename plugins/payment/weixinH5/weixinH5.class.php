...
class weixinH5 {
     ...
     */
    function get_code($order, $config, $return_url = '') {
        !$return_url && $return_url = SITE_URL . '/index.php/mobile/Payment/returnUrl/pay_code/weixinH5?order_sn=' . $order['order_sn'];
        ...
        if($return['return_code'] == 'SUCCESS' && $return['result_code'] == 'SUCCESS') {
            //        var_dump($return);
            $return['mweb_url'] .= '&redirect_url=' . $return_url;

            return '<script>window.location.href = "' . $return['mweb_url'] . '";</script>';
    ...
    function transfer($data) {
        ...
        if(!$res) {
            return ['status' => 1, 'msg' => "Can't connect the server"];
        }
