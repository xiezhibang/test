...
//        }
        ...
        $result = $this->mobileSmsApi($mobile, $smsSign, $smsParam, $templateCode,$msg);
     ...
     * @return mixed
     */
    public function mobileSmsApi($mobile, $smsSign, $smsParam, $templateCode,$msg){
        session('user.mobile',null);

//        if (is_null($mobile) && !preg_match('/^1([0-9]{9})/',$mobile)){
//            $this->r_data['code'] = 1;
//            $this->r_data['data'] = '参数错误';
//            return $this->foot();
//        }

//        if (time() < session('mobileSms')+30){
//            $this->r_data['code'] = 110;
//            $this->r_data['data'] = '请求过于频繁，30秒后再请求';
//            return $this->foot();
//        }

//        $Verify = new Verify(['length'=>4]);
//        $code   = $Verify->entry($type,1);
        $ch = curl_init();
        ...
        $data = [
            ...
            'Name'      =>'聚慧美桐荟',
            'Psw'       =>'123456',
//            'Message'   =>urlencode('【美桐荟】 您的验证码为'.json_decode($smsParam,true)['code'].'，请于3分钟内填写，如非本人操作，请忽略本短信'),
            'Message'   =>urlencode('【美桐荟】 '.$msg),
            'Phone'     =>$mobile,
            'Timestamp' =>0,
        ];
