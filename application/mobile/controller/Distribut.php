...
class Distribut extends MobileBase {
    ...
    */
    public function _initialize() {
        ...
        !$this->verifyLogin($nologin) && $this->error('请登录后再进行后继操作', U('Mobile/User/login'), '', 2);;
    ...
    public function lower_list($user_id = 0) {
        //        $this->error('功能维护中');
        if($user_id) {
            $user = Db::name('users')->find($user_id);
            $this->assign('title', $user['nickname']);
        } else {
            $user = session('user');
            $this->assign('lower_type', session('lower_type'));
        }
        ...
        $where = [
            'push_id' => $user['user_id'],
            //            'user_id' => ['NEQ',$user['user_id']],
            //            'distributor_ids'=>['LIKE','%u'.$user['user_id'].'%']
        ];

        if($user['distributor_level']) {
            $count_find = userFindCount($user['user_id'], $user['distributor_level'], $user['distributor_ids']);
            $this->assign('count_find', $count_find);
        }

        $count_child['level0'] = Db::name('users')->where($where)->where(['distributor_level' => 0])->count();
        $count_child['level1'] = Db::name('users')->where($where)->where(['distributor_level' => 1])->count();
        $count_child['level2'] = Db::name('users')->where($where)->where(['distributor_level' => 2])->count();
        $this->assign('count_child', $count_child);// 总人数
     ...
     * 下线列表(我的团队)
     */
    public function ajax_lower_list($user_id = 0, $type = 0) {
        if($user_id) {
            $user = Db::name('users')->find($user_id);
            $this->assign('title', $user['nickname']);
        }
        ...
        $lists = Db::name('users')
            ->field('nickname,user_id,distribut_money,reg_time,head_pic,is_vip,distributor_level,mobile')
            ->where($where)
            //            ->bind($bind)
...
//            $item['count_find'] = $item['distributor_level'] ?
//                userFindCount($item['user_id'], $item['distributor_level'], $item['distributor_ids']) : 0;
            $item['count_find'] = userFindCount($item['user_id'], $item['distributor_level'], $item['distributor_ids']);
        }
    ...
    public function my_store() {
        ...
        $goods_ids                          = $userDistributionModel->where(['user_id' => $this->user_id])->getField('goods_id', true);  //用户分销商品id
    ...
    public function rankings() {
        ...
        $lists          = Db::name('users')->where(['is_distribut' => 1])->order("$sort desc")->limit($Page->firstRow . ',' . $Page->listRows)->select(); //获排行列表
