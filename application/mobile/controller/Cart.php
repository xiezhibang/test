...
class Cart extends MobileBase
{
    ...
    public function  test(){
        Db::name('users')->where(['distributor_ids' => ['LIKE', '%u' . $item['user_id'] . '%'], 'is_vip' => 1])->count();
    }
...
//        if ($quota = $this->verifyVipGoods($goods_id,$goods_num)){
//            $this->ajaxReturn(['status'=>0,'msg'=>'您的任领额度不足，剩余￥'.$quota,'result'=>'']);
//        }

        $goods_ids = Db::name('goods')->where(['is_vip'=>-1])->column('goods_id');
        if(in_array($goods_id,$goods_ids)){
            //必须登录
            !$this->verifyLogin() && $this->ajaxReturn(['status'=>0,'msg'=>'必须登录才能购买该类商品','result'=>'']);

            //非VIP购买
            session('user.is_vip') && $this->ajaxReturn(['status'=>0,'msg'=>'您已经成为会员，无法购买该类商品','result'=>'']);

            //已存在订单记录
            $user_goods_ids = Db::name('order')->alias('o')
                ->where(['user_id'=>$this->user_id,'pay_status'=>0,'order_status'=>['neq',3],'goods_id'=>['IN',$goods_ids]])
                ->join('order_goods og','o.order_id = og.order_id')
                ->column('goods_id');
            array_intersect($user_goods_ids,$goods_ids) && $this->ajaxReturn(['status'=>-9,'msg'=>'您有未支付订单，无法重复购买','result'=>'']);

            //已存在购物车
            $user_cart_goods = Db::name('cart')->where(['user_id'=>$this->user_id,'goods_id'=>$goods_id])->find();
            $user_cart_goods && $this->ajaxReturn(['status'=>1,'msg'=>'您购物车已存在该类商品，无法重复购买','result'=>'']);
        }
...
//    //vip商品额度扣费
//    function verifyVipGoods($goods_id,$goods_num){
//        $goods = Db::name('goods')->where(['goods_id'=>$goods_id])->find();
//        if ($goods['is_vip'] != 1) return false; //非任领商品不继续过程
//
//        $user = session('user');
//        $quota = $user['quota']-($goods['market_price']*$goods_num);
//
//        if ($quota<0){
//            return $user['quota'];
//        }else{
//            session('user.quota',$quota);
//            Db::name('users')->where(['user_id'=>$user['user_id']])->update(['quota'=>$quota]);
//            return false;
//        }
//    }
    ...
    public function cart2()
    {
        ...
        if (empty($address)) {
            header("Location: " . U('Mobile/User/add_address', ['source' => 'cart2']));
            exit;
        } else {
...
//        $this->assign('couponList', $couponList); // 优惠券列表
        ...
        $this->assign('total_quota', $result['total_quota']); // 总计额度
        $this->assign('total_famous_quota', $result['total_famous_quota']); // 总计额度
...
//        $this->assign('checkconpon', $checkconpon); // 使用的优惠券
    ...
    public function cart3()
    {
        ...
        if ($this->user_id == 0) {
            exit(json_encode(['status' => -100, 'msg' => "登录超时请重新登录!", 'result' => null])); // 返回结果状态
        }
        ...
        $cartLogic->setUserId($this->user_id);
        if ($cartLogic->getUserCartOrderCount() == 0) {
            exit(json_encode(['status' => -2, 'msg' => '你的购物车没有选中商品', 'result' => null])); // 返回结果状态
        }
        if (!$address_id) exit(json_encode(['status' => -3, 'msg' => '请先填写收货人信息', 'result' => null])); // 返回结果状态
        if (!$shipping_code) exit(json_encode(['status' => -4, 'msg' => '请选择物流信息', 'result' => null])); // 返回结果状态

        $shipping_code = 'baishiwuliu';
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            if ($pay_points || $user_money) {
                if ($this->user['is_lock'] == 1) {
                    exit(json_encode(['status' => -5, 'msg' => "账号异常已被锁定，不能使用余额支付！", 'result' => null])); // 用户被冻结不能使用余额支付
                }
                if (empty($this->user['paypwd'])) {
                    exit(json_encode(['status' => -6, 'msg' => '请先设置支付密码', 'result' => null]));
                }
                if (empty($paypwd)) {
                    exit(json_encode(['status' => -7, 'msg' => '请输入支付密码', 'result' => null]));
                }
                if (encrypt($paypwd) !== $this->user['paypwd']) {
                    exit(json_encode(['status' => -8, 'msg' => '支付密码错误', 'result' => null]));
                }
            ...
            $user  = session('user');
            $quota = $user['quota']-$result['result']['quota'];
            if ($quota >= 0) {
                //                session('user.quota', $quota);
                //                Db::name('users')->where(['user_id' => $user['user_id']])->update(['quota' => $quota]);
            }else{
                exit(json_encode(['status' => -10, 'msg' => '您的任领余额不足', 'result' => null]));
            }
            $famous_quota = $user['famous_quota']-$result['result']['famous_quota'];
            if ($famous_quota >= 0) {
                //                session('user.quota', $quota);
                //                Db::name('users')->where(['user_id' => $user['user_id']])->update(['quota' => $quota]);
            }else{
                exit(json_encode(['status' => -10, 'msg' => '您的名品余额不足', 'result' => null]));
            }

            $orderLogic = new OrderLogic();
            $result     = $orderLogic->addOrder($this->user_id, $address_id, $shipping_code, $invoice_title, $coupon_id, $car_price, $user_note, $pay_name, $result['result']['quota'],$result['result']['famous_quota']); // 添加订单
            exit(json_encode($result));
        }
        $return_arr = ['status' => 1, 'msg' => '计算成功', 'result' => $car_price]; // 返回结果状态
    ...
    public function cart4()
    {
        ...
        if ($order['pay_status'] == 1) {
            $order_detail_url = U("Mobile/User/order_detail", ['id' => $order_id]);
        ...
        if (strstr($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
            ...
            if ($order['order_prom_type'] == 4 || $order['order_prom_type'] == 1) {
                ...
                $payment_where['code'] = ['in', ['weixin', 'cod']];
            }
        } else {
            if ($order['order_prom_type'] == 4 || $order['order_prom_type'] == 1) {
                //预售订单和抢购不支持货到付款
        ...
        if ($order['order_prom_type'] != 4) {
            ...
            if ($res['status'] == 1)
                $this->error('订单超时未支付已自动取消', U("Mobile/User/order_detail", ['id' => $order_id]));
        }
...
//        $result = $this->cartLogic->cartList($this->user, $this->session_id,1);
    ...
    public function header_cart_list()
    {
        ...
        if (empty($cart_result['total_price']))
            $cart_result['total_price'] = ['total_fee' => 0, 'cut_fee' => 0, 'num' => 0];
 ...
 */
    ...
    public function ajaxDelCart()
    {
        ...
        $return_arr = ['status' => 1, 'msg' => '删除成功', 'result' => '']; // 返回结果状态
        exit(json_encode($return_arr));
    }
