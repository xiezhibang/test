...
//        $this->assign('level_name', $level_name);
        ...
        $this->assign('distributor_level', ['VIP会员', '行政服务商', '白金服务商']);
        ...
        $half_month = strtotime('-14 day', $today_time);
        $rebate     = Db::name('rebate_log')->where(['user_id' => $user_id, 'status' => ['IN', [1, 2]], 'create_time' => ['EGT', $half_month]])->sum('money');
        $rebate += Db::name('account_log')->where(['user_id' => $user_id, 'change_time' => ['EGT', $half_month], 'user_money' => ['GT', 0]])->sum('user_money');

        $this->assign('rebate', $rebate ? $rebate : 0);
        //今日佣金
        $today_rebate = 0;
//        $today_rebate+= Db::name('rebate_log')->where(['user_id' => $user_id, 'status' => ['IN', [1, 2]], 'create_time' => ['EGT', $today_time]])->sum('money');
        $today_rebate += Db::name('rebate_log')->where(['user_id' => $user_id, 'status' => ['IN', [1, 2]], 'create_time' => ['EGT', $today_time]])->sum('money');
        $today_rebate += Db::name('account_log')->where(['user_id' => $user_id, 'change_time' => ['EGT', $today_time], 'user_money' => ['GT', 0]])->sum('user_money');
        $this->assign('today_rebate', $today_rebate);

//        $rebate_chart = [];
        $week_time    = strtotime('-6 day', $today_time);
        $rebate_chart = Db::name('account_log')->where(['user_id' => $user_id, 'change_time' => ['EGT', $week_time], 'user_money' => ['GT', 0]])->field(['change_time' => 'create_time', 'user_money' => 'money'])->select();
        $rebate_week  = [0, 0, 0, 0, 0, 0, 0];
        if ($rebate_chart) foreach ($rebate_chart as $item) {
            $key               = floor(($item['create_time'] - $week_time) / (60 * 60 * 24));
            $rebate_week[$key] += $item['money'];
        }
        $rebate_chart = Db::name('rebate_log')->where(['user_id' => $user_id, 'status' => ['IN', [1, 2]], 'create_time' => ['EGT', $week_time]])->field(['create_time', 'money'])->select();
        if ($rebate_chart) foreach ($rebate_chart as $item) {
            $key               = floor(($item['create_time'] - $week_time) / (60 * 60 * 24));
            $rebate_week[$key] += $item['money'];
        }

        $this->assign('rebate_week', json_encode($rebate_week));
        $this->assign('low_week', json_encode([
            date('m/d', strtotime('-6 day', $today_time)),
            '',
            //            date('m/d',strtotime('-5 day', $today_time)),
            date('m/d', strtotime('-4 day', $today_time)),
            '',
            //            date('m/d',strtotime('-3 day', $today_time)),
            date('m/d', strtotime('-2 day', $today_time)),
            '',
            //            date('m/d',strtotime('-1 day', $today_time)),
            date('m/d', $today_time),
        ]));
        $cumulative_money = db('rebate_log')->where(['user_id' => $user_id, 'status' => ['IN', [2, 3]]])->sum('money')
//            + db('account_log')->where(['user_id' => $user_id])->sum('user_money')
            + $user['result']['user_money'];
...
//            }
            ...
            $nickname  = I('nickname', '');
            if (!$distribut) $this->ajaxReturn(['status' => 0, '错误！页面无推荐者']);

            $data = $logic->reg($username, $password, $password2, trim($nickname), $distribut);
            if ($data['status'] != 1)
...
//        $this->success($data['msg']);
    ...
    public function add_comment()
    {
        if (IS_POST) {
            ...
            if ($row['status'] == 1) {
                $this->success('评论成功', U('/Mobile/User/comment', ['status' => 1]));
                exit();
            } else {
                $this->error($row['msg']);
            }
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            if ($_FILES['head_pic']['tmp_name']) {
                ...
                if ($info) {
                    $post['head_pic'] = '/' . $dir . $parentDir . '/' . $info->move($dir, true)->getFilename();
                } else {
...
//            header("Location: " . U('User/Index'));
    ...
    public function return_goods()
    {
        ...
        if (!empty($return_goods)) {
            $this->error('已经提交过退货申请!', U('Order/return_goods_info', ['id' => $return_goods['id']]));
        }
        $order_goods = M('order_goods')->where(['rec_id' => $rec_id])->find();
...
//            }
            ...
            //提现时间为每月10，25号下午5.30前，每个阶段只有一次提现申请，后台确认发放后，才可以二次提现申请
            $d = date('d');
            if ($d >= 25) {
                $sta_time = strtotime(date('Y-m') . '-25 17:30');
                $end_time = strtotime('+1 month +9 day', strtotime(date('Y-m') . '-1 17:30'));
            } else if (25 > $d && $d >= 10) {
                $sta_time = strtotime('+9 day', strtotime(date('Y-m') . '-1 17:30'));
                $end_time = strtotime('+24 day', strtotime(date('Y-m') . '-1 17:30'));
            } else {
                $sta_time = strtotime('+9 day', strtotime(date('Y-m') . '-1 17:30'));
                $end_time = strtotime('-1month +24 day', strtotime(date('Y-m') . '-1 17:30'));
            }
            $result = Db::name('withdrawals')->where(['user_id' => $this->user_id, 'create_time' => ['EGT', $sta_time]])->where(['create_time' => ['LT', $end_time]])->find();
            if ($result) $this->error('您同一时间段内已提交过提现申请:(提现时间为每月10，25号下午5.30前，每个阶段只有一次提现申请，且后台确认发放后，才可以二次提现申请)');

            $data['meibi'] = $data['money'];
            $data['money'] *= 0.8;
...
//            $data['money'] *=0.8;
//
//            if (Db::name('withdrawals')->add($data)) {
//                $this->success("已提交申请");
//                exit;
//            } else {
...
//    }
    ...
    public function return_goods_cancel()
    {
        ...
        M('return_goods')->where(['id' => $id])->save(['status' => -2, 'canceltime' => time()]);
    ...
    public function receiveConfirm()
    {
        ...
        if ($update) {
            M('order_goods')->where([
                ...
                'spec_key' => $return_info['spec_key'],
            ])->save(['is_send' => 2]);  //订单商品改为已换货
            $this->success("操作成功", U("User/return_goods_info", ['id' => $return_id]));
        }
        $this->error("操作失败");
    }
