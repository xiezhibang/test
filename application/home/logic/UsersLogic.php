...
//			}
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$nickname='',$push_id=0){
    	$is_validated = 0 ;
        ...
        if (empty($nickname)){
            return array('status'=>-1,'msg'=>'请输入用户名称');
        }
        $map['nickname'] = $nickname;

        if($is_validated != 1)
            return array('status'=>-1,'msg'=>'请输入手机号注册');
...
//        $map['first_leader'] = cookie('first_leader'); // 推荐人id
        $map['first_leader'] = $push_id; // 推荐人id
//        // 如果找到他老爸还要找他爷爷他祖父等
//        if($map['first_leader'])
//        {
//            $first_leader = M('users')->where("user_id", $map['first_leader'])->find();
...
//		}
        ...
        //分享链id追踪
        $distributor_ids = 'u'.$user_id.',';
        if ($push_id){
            $push = Db::name('users')->where(['user_id'=>$push_id])->find();
            $distributor_ids = $push['distributor_ids'].$distributor_ids;
        }

        Db::name('users')->update(['user_id'=>$user_id,'distributor_ids'=>$distributor_ids]);
...
//        }
    ...
    public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
        ...
        if($order['pay_status'] > 0)
            return array('status'=>-1,'msg'=>'订单已支付,请联系客服处理退款','result'=>'');
...
//		if ($order['quota'] > 0){
//		    Db::name('users')->where(['user_id'=>$user_id])->setInc('quota',$order['quota']);
