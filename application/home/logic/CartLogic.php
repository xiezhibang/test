...
class CartLogic extends Model
{
    ...
    private function addNormalCart($goods_num, $goods_spec_key)
    {
        ...
        if ($userCartGoods) {
            ...
            if ($userWantGoodsNum > $this->goods['store_count']) {
                return ['status' => -4, 'msg' => '商品库存不足，剩余' . $this->goods['store_count'] . ',当前购物车已有' . $userCartGoods['goods_num'] . '件', 'result' => ''];
            }
            //如果有阶梯价格
    ...
    private function addFlashSaleCart($goods_num, $goods_spec_key)
    {
        ...
        if ($flashSaleIsEnd) {
            return ['status' => -9, 'msg' => '秒杀活动已结束', 'result' => ''];
        }
        ...
        if ($userWantGoodsNum > $flashSalePurchase) {
            return ['status' => -4, 'msg' => '商品库存不足，剩余' . $flashSalePurchase . ',当前购物车已有' . $userCartGoodsNum . '件', 'result' => ''];
        }
    ...
    private function addGroupBuyCart($goods_num)
    {
        ...
        if ($groupBuy['is_end'] == 1 || empty($groupBuy)) {
            return ['status' => -4, 'msg' => '团购活动已结束', 'result' => ''];
        }
        //获取用户购物车的团购商品
        $userCartGoods    = $CartModel::get(['user_id' => $this->user_id, 'session_id' => $this->session_id, 'goods_id' => $this->goods['goods_id']]);
    ...
    public function getUserCartList($selected)
    {
        ...
        $total_goods_num = $total_price = $total_quota = $total_famous_quota = $cut_fee = 0;//初始化数据。商品总共数量/商品总额/节约金额
        foreach ($cartList as $k => $val) {
            ...
            if ($goods = Db::name('goods')->where(['goods_id' => $val['goods_id'], 'is_vip' => 1])->find())
                $total_quota += $val['market_price'] * $val['goods_num'];

            if ($goods = Db::name('goods')->where(['goods_id' => $val['goods_id'], 'is_vip' => 2])->find())
                $total_famous_quota += $val['market_price'] * $val['goods_num'];
        }

        $total_price = ['total_fee' => $total_price, 'cut_fee' => $cut_fee, 'num' => $total_goods_num,]; // 总计
        setcookie('cn', $total_goods_num, null, '/');

        return ['cartList' => $cartList, 'total_price' => $total_price,'total_quota'=>sprintf("%.2f",$total_quota),'total_famous_quota'=>sprintf("%.2f",$total_famous_quota)];
    }
