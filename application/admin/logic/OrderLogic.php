...
class OrderLogic extends Model {
    ...
    public function deliveryHandle($data) {
        ...
        $shipping               = M('plugin')->where(['status' => 1, 'type' => 'shipping', 'code' => $data['shipping_code']])->find();
        ...
        $updata['shipping_code'] = $shipping['code'];
        ...
        $s = $this->orderActionLog($order['order_id'], 'delivery', $data['note'] . '|快递单号:' . $data['invoice_no']);//操作日志
        ...
        if($res && $res['status'] == 1) {
            ...
            if($users) {
                ...
                $params   = ['user_name' => $nickname, 'order_sn' => $data['order_sn'], 'consignee' => $data['consignee']];
    ...
    public function getAddressName($p = 0, $c = 0, $d = 0) {
        $p = M('region')->where(['id' => $p])->field('name')->find();
        $c = M('region')->where(['id' => $c])->field('name')->find();
        $d = M('region')->where(['id' => $d])->field('name')->find();
    ...
    function delOrder($order_id) {
        $a = M('order')->where(['order_id' => $order_id])->delete();
        $b = M('order_goods')->where(['order_id' => $order_id])->delete();
