...
class Goods extends Base
{
    ...
    public function addEditCategory()
    {
        ...
        if (I('is_ajax') == 1) {
            ...
            if (!$validate->batch()->check(input('post.'))) {
                ...
                if ($type == 2) {
                    $children_where = [
                        'parent_id_path' => ['like', '%_' . I('id') . "_%"],
                    ];
                    $children       = M('goods_category')->where($children_where)->max('level');
                    if (I('parent_id_1')) {
                        $parent_level = M('goods_category')->where(['id' => I('parent_id_1')])->getField('level', false);
                    ...
                    if (I('parent_id_2')) {
                        $parent_level = M('goods_category')->where(['id' => I('parent_id_2')])->getField('level', false);
                        if (($parent_level + $children) > 4) {
                            $return_arr = [
                                'status' => -1,
                                'msg'    => '商品分类最多为三级',
     ...
     *  商品列表
     */
    public function upVipGoodsList()
    {
        $this->assign('is_vip', -1);
        return $this->fetch();
    }
...
//        $where = ' 1 = 1 '; // 搜索条件
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            // 数据验证
            $is_vip = input('is_vip');
            $url = ['admin/Goods/goodsList','admin/Goods/vipGoodsList',-1=>'admin/Goods/upVipGoodsList'];
            $return_url   = url($url[$is_vip]);
            $data         = input('post.');
...
//            print_r('<pre>');var_dump($Goods);die;
            ...
            $price_ladder             = [];
            if (isset($Goods->ladder_amount) && $Goods->ladder_amount[0] > 0) {
            ...
            if ($type == 2) {
                ...
                // 修改商品后购物车的商品价格也修改一下
                M('cart')->where("goods_id = $goods_id and spec_key = ''")->save([
        ...
        $plugin_shipping         = M('plugin')->where(['type' => ['eq', 'shipping']])->select();//插件物流
        ...
        $is_vip = $goodsInfo['is_vip'] ? $goodsInfo['is_vip'] : I('is_vip');
        return $this->fetch($is_vip == -1 ? 'upVipGoods' : '_goods');
    }
    ...
    public function delGoods()
    {
        ...
        if ($error) {
            $return_arr = ['status' => -1, 'msg' => $error, 'data' => '',];   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);
            $this->ajaxReturn($return_arr);
        }
        ...
        $return_arr = ['status' => 1, 'msg' => '操作成功', 'data' => '',];   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);
        $this->ajaxReturn($return_arr);
    }
    ...
    public function delBrand()
    {
        ...
        if ($goods_count) {
            $return_arr = ['status' => -1, 'msg' => '此品牌有商品在用不得删除!', 'data' => '',];   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);
            $this->ajaxReturn($return_arr);
        }
    ...
    {
        $this->assign("URL_upload", U('admin/Ueditor/imageUp', ['savepath' => 'goods'])); // 图片上传目录
        $this->assign("URL_imageUp", U('admin/Ueditor/imageUp', ['savepath' => 'article'])); //  不知道啥图片
        $this->assign("URL_fileUp", U('admin/Ueditor/fileUp', ['savepath' => 'article'])); // 文件上传s
        $this->assign("URL_scrawlUp", U('admin/Ueditor/scrawlUp', ['savepath' => 'article']));  //  图片流
        $this->assign("URL_getRemoteImage", U('admin/Ueditor/getRemoteImage', ['savepath' => 'article'])); // 远程图片管理
        $this->assign("URL_imageManager", U('admin/Ueditor/imageManager', ['savepath' => 'article'])); // 图片管理
        $this->assign("URL_getMovie", U('admin/Ueditor/getMovie', ['savepath' => 'article'])); // 视频上传
    ...
    public function addEditSpec()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST)//ajax提交验证
        {
            ...
            $return_arr = [
                'status' => 1,
                'msg'    => '操作成功',
                'data'   => ['url' => U('Admin/Goods/specList')],
            ];
            $this->ajaxReturn($return_arr);
        }
        // 点击过来编辑时
