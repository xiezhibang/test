...
//        $sort_order = I('order_by').' '.I('sort');
        ...
        $this->assign('level',['VIP会员','行政服务商','白金服务商']);
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            $data = $_POST;
            //任领额度计算
//            if ($user['distributor_level'] != $data['distributor_level']){
//                switch ($data['distributor_level']){
//                    case 1:
//                        $data['quota'] = $user['quota'] + 20000;
//                        break;
//                    case 2:
//                        $data['quota'] = $user['quota'] + 50000;
//                        break;
//                    default:
//                        break;
//                }
//            }
        ...
        foreach($user_grandparents_child_list as $item) {
            if(Db::name('users')->where(['distributor_ids' => ['LIKE', '%u' . $item['user_id'] . '%'], 'is_vip' => 1])->count() >= 5) {
                $child_count30++;
            };
        }
        $this->assign('count1',$user_child_count);
        $this->assign('count2',$child_count30);
        $this->assign('count3',$user_find_count);
        return $this->fetch();
    }
    ...
    public function account_edit(){
        ...
        $user = M('users')->field('user_id,user_money,frozen_money,pay_points,is_lock,quota,famous_quota')->where('user_id',$user_id)->find();
        if(IS_POST){
            $desc = I('post.desc');
            if(!$desc)
