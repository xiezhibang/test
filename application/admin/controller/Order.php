...
class Order extends Base {
    ...
    */
        ...
        if($data['pay_status'] == 3 && $data['refund_type'] == 0) {
            ...
            if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile') {
                ...
                if($order['pay_code'] == 'weixin') {
                    ...
                    $data        = ['transaction_id' => $order['transaction_id'], 'total_fee' => $order['order_amount'], 'refund_fee' => $return_money];
                    ...
                    $data        = ['batch_no' => date('YmdHi') . $order['order_id'], 'batch_num' => 1, 'detail_data' => $detail_data];
    ...
    public function detail($order_id) {
        ...
        // 获取操作记录
        $action_log = M('order_action')->where(['order_id' => $order_id])->order('log_time desc')->select();
        $has_user   = false;
        $adminIds   = [];
        //查找用户昵称
    ...
    public function edit_order() {
        ...
        if(IS_POST) {
            ...
            $order['shipping_name'] = M('plugin')->where(['status' => 1, 'type' => 'shipping', 'code' => I('shipping')])->getField('name');
        ...
        //获取订单城市
        $city = M('region')->where(['parent_id' => $order['province'], 'level' => 2])->select();
        //获取订单地区
        $area = M('region')->where(['parent_id' => $order['city'], 'level' => 3])->select();
        //获取支付方式
        $payment_list = M('plugin')->where(['status' => 1, 'type' => 'payment'])->select();
        //获取配送方式
        $shipping_list = M('plugin')->where(['status' => 1, 'type' => 'shipping'])->select();
    ...
    public function shipping_print() {
        ...
        $order      = $orderLogic->getOrderInfo($order_id);
        //查询是否存在订单及物流
        $shipping = M('plugin')->where(['code' => $order['shipping_code'], 'type' => 'shipping'])->find();
        if(!$shipping) {
            $this->error('物流插件不存在');
        }
        ...
        if(empty($shipping['config'])) {
            $config = ['width' => 840, 'height' => 480, 'offset_x' => 0, 'offset_y' => 0];
            $this->assign('config', $config);
        } else {
            $this->assign('config', unserialize($shipping['config']));
        }
        $template_var             = [
            "发货点-名称", "发货点-联系人", "发货点-电话", "发货点-省份", "发货点-城市",
        ...
        $content_var              = [
            $shop['store_name'], $shop['contact'], $shop['phone'], $shop['province'], $shop['city'],
    ...
    public function deliveryHandle() {
        ...
        if($res) {
            $this->success('操作成功', U('Admin/Order/detail', ['order_id' => $data['order_id']]));
        } else {
            $this->success('操作失败', U('Admin/Order/delivery_info', ['order_id' => $data['order_id']]));
        }
    }
    ...
    public function ajax_return_list() {
        ...
        empty($order_sn) && is_numeric($status) && $where .= " and status = '$status' ";
...
//                		}else{
            ...
            //    			$payment_obj = new \alipay();
            $detail_data = $order['transaction_id'] . '^' . $return_money . '^' . '用户申请订单退款';
            $data        = ['batch_no' => date('YmdHi') . $rec_goods['rec_id'], 'batch_num' => 1, 'detail_data' => $detail_data];

            $goods_quota = Db::name('goods')->where(['goods_id'=>$return_goods['goods_id']])->value('market_price');
            accountLog($return_goods['user_id'], $return_goods['refund_money'], $goods_quota*$return_goods['goods_num'], '用户申请商品退款', 0, $return_goods['order_id'], $return_goods['order_sn']);
    ...
    public function order_action() {
        ...
        if($action && $order_id) {
            ...
            if($res !== false && $a !== false) {
                if($action == 'remove') {
                    exit(json_encode(['status' => 1, 'msg' => '操作成功', 'data' => ['url' => U('admin/order/index')]]));
                }
                exit(json_encode(['status' => 1, 'msg' => '操作成功']));
            } else {
                if($action == 'remove') {
                    exit(json_encode(['status' => 0, 'msg' => '操作失败', 'data' => ['url' => U('admin/order/index')]]));
                }
                exit(json_encode(['status' => 0, 'msg' => '操作失败']));
            }
        } else {
            $this->error('参数错误', U('Admin/Order/detail', ['order_id' => $order_id]));
        }
    }
        ...
        if(is_array($orderList)) {
            ...
            foreach($orderList as $k => $val) {
                ...
                foreach($orderGoods as $goods) {
                    ...
                    $strGoods  .= "商品编号：" . $goods['goods_sn'] . " 商品名称：" . $goods['goods_name'] . " x " . $goods['goods_num'];
    ...
    public function add_order() {
        ...
        if(IS_POST) {
            ...
            if($order_id) {
                ...
                $this->success('添加商品成功', U("Admin/Order/detail", ['order_id' => $order_insert_id]));
                exit();
            } else {
                $this->error('添加失败');
            }
